...
class AnalysisTransform(nn.Module):
    def __init__(self, image_dims, num_filters=SMALL_FILTERS,
                 C=SMALL_BOTTLENECK):
        """ 
        Encoder with convolutional architecture used in [1].
        Projects image x ([C_in,256,256]) into a feature map of size C x W/16 x H/16
        ========
        Arguments:
        image_dims:  Dimensions of input image, (C_in,H,W)
        ...
        # Layer / normalization options
        cnn_kwargs = dict(stride=2, padding=kernel_dim//2, padding_mode='reflect')

        self.pre_pad = nn.ReflectionPad2d(3)
        self.n_downsampling_layers = 4
        filters = [num_filters for _ in range(self.n_downsampling_layers)]
        ...
        # (32,32) -> (16,16)
        self.conv_block4 = nn.Sequential(
            nn.Conv2d(filters[2], C, kernel_dim, **cnn_kwargs),
        )
...
if __name__ == "__main__":
    ...
    A = AnalysisTransform(image_dims=x_dims[1:], C=C)
