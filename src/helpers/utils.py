...
def log(model, storage, epoch, idx, mean_epoch_loss, current_loss, best_loss, start_time, epoch_start_time, 
    ...
    if model.iw is True:
        ...
        report_f("n_marginal_bpp: {:.3f} | q_marginal_bpp: {:.3f} | Inference grad var: {:.3f} | Inference grad SNR: {:.3f}".format(storage['n_latent_marginal'][-1],
                storage['q_latent_marginal'][-1], storage['inference_grad_var'][-1], storage['inference_grad_SNR_sq'][-1]))
